...
public class RequestThread extends Thread {
    ...
    public RequestThread(SimpleWebServer sWS, Socket socket, File rootDir) {
	 parent = sWS;
        _socket = socket;
        _rootDir = rootDir;
	  utsqad = new UtilSQLAdapter();
    }
    ...
    public void run() {
        ...
        try {
         ...
         System.out.println("RequestThread: Invalid Method");
                sendError(out, 500, "Invalid Method.");

                return;
            }            
            String npath = request.substring(4, request.length() - 9);    
            String path = npath;    

            if(npath.contains("?")) {
                // System.out.print("onPageStarted:oride " + url);
                String[] temp;
                temp = npath.split(Pattern.quote("?"));
 		    path = temp[0];
		   // parent.sndToParent(temp[1]);
            }
         System.out.println("RequestThread: nupath: " + path);        
            File file = new File(_rootDir, URLDecoder.decode(path, "UTF-8")).getCanonicalFile();
 	 
         System.out.println("RequestThread: file: " + file.toString());     
            if (file.isDirectory()) {
         ...
         System.out.println("RequestThread: Permission Denied");
                sendError(out, 403, "Permission Denied.");
            }
            else if (!file.exists()) {
                // The file was not found.
         System.out.println("RequestThread: File Not Found");
			...
			if((path.contains("app.pst")) && (npath.contains("?"))) {
 
                String[] tempa;
                tempa = npath.split(Pattern.quote("?"));
 		    String qpath = URLDecoder.decode(tempa[1], "UTF-8");
		    String tmpGBstr = utsqad.setDBselectQ(qpath);
                 out.write((tmpGBstr).getBytes());
         ...
         System.out.println("RequestThread error: " + anye.toString());
                    // Do nothing.
                }
            }
        }
    ...
    SimpleWebServer sWS;
    SimpleWebServer parent;
    private File _rootDir;
    private Socket _socket;
    UtilSQLAdapter utsqad;
}